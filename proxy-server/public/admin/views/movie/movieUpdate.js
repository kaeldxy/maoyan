...
export default class {
	constructor(el) {
		...
		this.template =
		`
		<div class="movieUpdate-page">
			<form class="layui-form layui-form-pane" lay-filter="movieUpdate">
				...
				<div class="layui-form-item">
					<div class="layui-input-block">
					<button type="button" class="layui-btn" lay-submit lay-filter="movieUpdate">立即提交</button>
	...
	handle() {
		var movieData = JSON.parse(localStorage['movieupdatedata']);
		localStorage.removeItem('movieData');
		layui.form.render();
		movieData.type1 = movieData.type[0];
		movieData.type2 = movieData.type[1];
		
		layui.form.val('movieUpdate', movieData);
		$('.show-movie-poster').attr('src', '../' + movieData.poster);
		$('.show-movie-poster').html(
			movieData.episode
				.map(item => `<img src="../${item}" alt=""  style="width:120px; height:80px;">`)
				.join('')
		)
		...
		layui.form.on('submit(movieUpdate)', function (data) {
			$('.current-state').each(function () {
				const key = this.dataset.state;
				movieData[key] = this.checked;
			})
			delete data.field['movie-episode-files'];
			data.field.type = [data.field.type1, data.field.type2];
			
			Object.assign(movieData, data.field);
			delete movieData.type1;
			delete movieData.type2;
			movieData.type = JSON.stringify(movieData.type);
			movieData.episode = JSON.stringify(movieData.episode);
			$.ajax({
